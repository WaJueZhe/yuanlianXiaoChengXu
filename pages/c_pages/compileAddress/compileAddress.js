// pages/c_pages/compileAddress/compileAddress.js
...
Page({
  ...
  editSubmit(){
    ...
    switch (wx.getStorageSync('identityType')) {
      case 'pcuser':
        addUrl = `${api.baseurl}/address/updateAddress`
        break;
      case 'customer':
        addUrl = `${api.baseurl}/cuserAddress/updateAddress`
        break;
      default:
        break;
    }
  ...
  onShow: function () {
    this.setData({
      id: app.addressList.id,//地址类型
      customerOrgId: app.addressList.customerOrgId,//地址类型
      addressType: app.addressList.addressType,//地址类型
      addressCode: app.addressList.addressCode,//地址编码
      street: app.addressList.street,//详细地址
      currentContactPerson: app.addressList.currentContactPerson,//联系人
      currentContactMobile: app.addressList.currentContactMobile,//联系电话
      addressName: app.addressList.addressName,//单位名称

      province: app.addressList.provinceName,//省
      city: app.addressList.cityName,//市
      county: app.addressList.districtName,//区

      provinceId: app.addressList.provinceCode,//省ID
      cityId: app.addressList.cityCode,//市ID
      countyId: app.addressList.districtCode,//区ID

      pcdName: app.addressList.provinceName + ',' + app.addressList.cityName + ',' + app.addressList.districtName,
      pcdCode: app.addressList.provinceCode + ',' + app.addressList.cityCode + ',' + app.addressList.districtCode,
    })

    if (app.addressList.addressType == 'D'){
      this.setData({
        addressGenre: this.data.arr[0].name
      })
    }
    else {
      this.setData({
        addressGenre: this.data.arr[1].name
      })
    }

    this.data.pcdName = this.data.pcdName.split(",");
    this.data.pcdCode = this.data.pcdCode.split(",");

    console.log(this.data.pcdName)
    console.log(this.data.pcdCode)
  },
